...
- **`GripCorner.n` is a per-lap detection index, never a track turn ID.** On ten laps of the same physical circuit, corner detection finds anywhere from **6 to 9** corners, so lap 3's "corner 5" and lap 1's "corner 5" are different bends. `compare.ts` builds canonical turns by clustering apexes on the shared spatial axis instead. **Known consequence:** `bestApexG` in `grip-session-screen.tsx` still keys the "best at this corner across laps" reference on `c.n`, so its `spareScore` flags can compare unrelated corners. Fixing it means adopting the spatial turn identity there too.
...
- **Canvas draw code does not run in jsdom** — `getBoundingClientRect` returns a zero-size box, so `useCanvasDraw`'s `fitCanvas` bails and every `draw()` is skipped. A component test that renders a chart proves nothing about its drawing. `tests/ui/compare-canvas.test.tsx` stubs the rect to force the draws to execute.

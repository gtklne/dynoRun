...
**Hands-free capture (motorcycles)** is a whole second path through the same domain, because a rider cannot touch the phone mid-pull. Both halves of the workflow have one, and the rule for both is the same: **nothing is captured while the vehicle is moving, everything is chosen afterwards.**

- **Runs**: `SessionController` (`/vehicles/:v/calibrations/:c/session`) buffers the whole ride with no DB writes, then `detectPulls` (`src/analysis/pull-detection.ts`) finds the acceleration pulls and the rider saves the ones they meant. A session that is never saved costs nothing.
- **Calibration**: `CalibrationSessionController` buffers the whole ride, then `detectPlateaus` (`src/analysis/plateau-detection.ts`) finds every steady-speed hold and the rider picks the one that was their deliberate attempt at the target RPM. **Do not "improve" this into an auto-capture.** The interactive wizard latches on the *first* steady window it sees, which on any real ride is the cruise out to the test road in the wrong gear, and the rider cannot redo it without stopping. Deferring the choice is what makes the mode safe: a wrong-gear plateau becomes one more row to ignore rather than a silently wrong rollout that then divides into every future run's RPM axis.
- `detectPlateaus` uses the smoothed grid **only to locate** windows and re-measures spread and mean from the **raw** samples inside each one. Savitzky-Golay with window 9 on a 200 ms grid averages 1.8 s, so it irons real jitter flat and would pass a cruise as a hold; it also leaves the first and last `m` points untouched, which would make an edge plateau behave unlike an interior one. `CalibrationStabilityDetector.check` averages raw speed for the same reason.
- **Two guards, not one, keep `resample` from inventing a plateau.** It interpolates straight through a GPS dropout with no gap ceiling, and `performance.now()` keeps advancing while the app is suspended, so a suspend/resume fabricates a stretch of collinear points whose spread is exactly 0 and whose score is therefore a perfect 1.0, outranking every real hold. `min_raw_coverage_hz` (0.5) alone is **not enough**: it is blind to any hole shorter than half the window, and 30 s missing from a 60 s hold still reads 0.53 Hz. `max_raw_gap_ms` (4000, window edges counted as bracketing fixes) is the check that actually catches it. Both are pinned in `tests/analysis/plateau-detection.test.ts`, including a case proving the rate floor alone lets it through.
- Scoring caps the duration term at `score_duration_cap_ms` (15 s) so a 2-minute motorway cruise cannot outrank a deliberate 15 s hold on length alone; past the cap only tightness separates candidates.
- `CalibrationCandidate.plausible` is a **sanity range only** (`PLAUSIBLE_ROLLOUT_M_PER_REV`, 0.05-1.5 m/rev) and the UI must not imply otherwise: a hold in the wrong gear lands comfortably inside the band. It rules out nonsense, never a wrong pick.
- The live `CalibrationStabilityDetector` inside `CalibrationSessionController` **decides nothing**: it exists purely so `onHoldConfirmed` can speak "Steady at 90" mid-ride, which is the rider's only confirmation that a hold registered. A cue on incidental cruising costs a spoken number, nothing more. `HOLD_CUE_DEDUPE_KMH` stops a steady cruise calling out every few seconds.

**Grip analysis** (`src/analysis/grip/`) estimates demand from RaceBox speed and lean. `alongRaw = (dv/dt)/9.80665`; `along` adds a generic resistance model (rho 1.20 kg/m³, CdA 0.40 m², mass 260 kg, Crr 0.015; zero at standstill). `alat = tan(lean)` assumes steady balanced cornering and level ground. These are model estimates, not calibrated tyre-force or capacity measurements. Do not claim spare grip, riding safety, causal line/throttle advice, or universal comparability across bikes and conditions. Demand is the default; `load` is a persisted legacy mode name for the optional **activity index**, sqrt(comb² + (tau * rate)²). The rate differentiates vehicle-aligned components; it is not inertial jerk or suspension travel. The 50/50 point-mass axle estimate exposes the 0/100% lift boundaries where its assumptions fail.

**Observed envelope**: 72 directions, duration-weighted p95, at least 0.5 s support per bin, minimum speed selection and timed laps when available. These are explicit engineering/statistical choices. Unknown bins stay NaN, never filled from neighbouring directions. Full-circle/sector RMS requires all 72/18 bins respectively. More data can raise or lower quantiles. Equal lap budgets do not eliminate condition or duration bias. `anchorG` is only a colour/display scale. `spareScore` is a legacy stored key for an observed demand-difference flag. Apex = median within ±0.12 s; peak = actual maximum derived value.

**Timestamps and storage**: filtering and playback use recorded time; split at gaps greater than 1.5 median sample intervals and at invalid fixes. The legacy `speedSmooth` setting maps to (value - 1)/25 seconds. Stored raw data is version 2, including optional per-sample fix validity and counters. Versions 1 and 2 load through the same exhaustive client/API validator; old lost quality provenance cannot be recovered. No derived channels are persisted. Analysis revision 2 changes results on reload. See `docs/audits/2026-09-18-grip-implementation.md` and run `node scripts/audit-grip.mjs` for current regression checks.

**Track turns and lap comparison**: turn IDs are heuristic spatial clusters, partitioned by lean direction. They depend on the chosen laps/reference and are not a surveyed track map. Never pair corners by per-lap `GripCorner.n`. Single-session and comparison maps share WGS84 local scales. Projection plateaus use the last sample consistently. Comparisons use the longest contiguous supported shared section; all metric/time channels outside it are NaN, including when the reference has a gap. A complete finish delta requires actual endpoint support. Pace uses each lap's own path duration. Duty integrates clipped reference-axis cells and threshold crossings, not each subject's own travelled metres. The near-zero demand band cannot identify throttle/brake state. Segment times and metadata lap times use different boundaries: do not subtract one from the other. Comparison categories describe time and demand differences without claiming their cause. API/frontend version 2 must deploy together.
...
**Calibration stability:** `CalibrationStabilityDetector` requires ±1 km/h over a 5 s window **and a mean above `min_speed_kmh` (10)**. That floor is not hygiene: standing still is perfectly stable, so without it tapping "Start measurement" while parked captured a rock-steady 0 km/h after 5 s, latched to `stable`, stopped listening, and then failed the POST with a 400 because the server rejects `speed_kmh <= 0`. The floor is on the **mean** (the value `check()` returns and the wizard posts), not on the slowest sample: the delta check already bounds the spread, so keying on the minimum would make the effective floor `min_speed_kmh + max_speed_delta_kmh` and couple two independent settings. `progress()` still reports truthfully while parked and exposes `below_min_speed`, because the panel must say "too slow to calibrate" rather than show a bar that fills and then sits there.

**Standstill auto-finish:** `StandstillDetector` ends a hands-free recording after `standstill_ms` (20 s) at or below `stopped_speed_kmh` (5 km/h), so detection has already run by the time the phone comes out of the rider's pocket. **Arming is the whole design**: nothing can fire until a sample has exceeded `arm_speed_kmh` (25 km/h), because a recording started in the garage sits at 0 km/h for as long as it takes to put gloves on. Arming latches and is never cleared by a later slow sample. 5 km/h is tight enough because GNSS derives speed from Doppler shift rather than by differencing positions, so a stationary phone reads near zero; haversine differencing, which does jitter, is only the fallback for a platform reporting no speed at all.

**`SensorWatchdog` is the wall clock, and it is load-bearing.** Every other way a hands-free recording ends is driven by an arriving sample: the standstill detector, the max-duration check, the rider's hold button. So a sensor that simply stops delivering (permission revoked mid-ride, or the webview suspended after a wake lock was denied) left the session in `recording` forever with a button press as the only escape, which is precisely what hands-free promises the rider will not need. **The 30-minute `MAX_SESSION_DURATION_MS` cap was sample-driven too, so it could not save it either.** Both hands-free controllers own one; it runs off `Date.now()`, `beat()` is called for every sample, and firing is latched.
...
| `/vehicles/:vehicleId/calibrations/new` | 3-step wizard: gear → measure → confirm. The measure step has **two capture modes** picked on the gear step (`MeasureMode`): `tap` (watch the screen, confirm the lock) and `hands_free` (record the ride, pick a hold afterwards). Defaults to `hands_free` for `kind === 'motorcycle'` |
...
- **`CalibrationStepGear` seeds its capture-mode toggle with `useState`, which reads its initial value once.** So `CalibrationWizardScreen` must not mount the gear step before the vehicle fetch settles, or every vehicle silently pins to the `tap` default and motorcycles lose the hands-free default. The wizard renders "Loading vehicle" until then, and `tests/ui/calibration-handsfree.test.tsx` pins it with a deliberately unresolved promise.
- **`emitRide` in `tests/run/session-controller.test.ts` coasts down to 16.2 km/h, not to zero, on purpose.** That is above `DEFAULT_STANDSTILL_CONFIG.stopped_speed_kmh`, so no test built on that ride trips the standstill auto-finish. Lengthen the coast past ~28 s and every assertion downstream quietly becomes an auto-finish test instead.
- **`SpeedSource.errors$` is optional on the interface but was published-and-never-read before hands-free.** Both GPS sources had one, with two *different* payload shapes (`GeolocationPositionError` vs `{message}`), and nothing subscribed. It is now one `SensorError`. Both hands-free screens render it as a **persistent banner, never a toast**: the phone is in a pocket when it fires, so a toast is guaranteed to be missed, and the message has to still be on screen when the rider pulls the phone out.
...
- **Retired domains answer 410, and that config lives in the repo at `ops/nginx/retired-domains.conf`.** Four dead projects still point at this box: `whatsgoodin.ch`, `weekendwolf.app`, `weekendwombat.com`, `partydna.ch` (each with `www.`). The file is the source of truth, hand-applied as `/etc/nginx/sites-available/retired-domains` and symlinked into `sites-enabled`. Like the `/` and `/hello` blocks above, **CI never deploys it**: `scp` it up, `nginx -t`, reload. Backup of the pre-410 state: `/root/retired-domains.bak.pre-acme`.
  - **A 410, not a redirect and not a dead socket.** `wasgoht.ch/` was stuck in a duplicate cluster whose elected canonical was a `whatsgoodin.ch` URL, and Google will not drop a canonical it cannot fetch, since unreachable reads as temporarily-failing rather than gone. A fetchable permanent-removal signal is the thing that lets the cluster dissolve, so every retired name has to *answer*. Redirecting them to `/hello` would be worse than doing nothing: it hands the retired domain's history to the exact page the `/hello` move exists to keep clean.
  - **`robots.txt` deliberately returns 410 too.** Blocking it would stop the crawl that observes the removal, so the 410 must cover every path with no exceptions. This is why the `return 410` sits at **server level** rather than in a `location`: at server level it is evaluated in the server-rewrite phase, before nginx picks a location, so no path can escape it.
  - **Each name needs a 443 block of its own, or HTTPS silently breaks.** Without one, a request falls through to the default 443 server, gets handed the `wasgoht.ch` certificate (name mismatch, so a crawler stops at the handshake, which is the unreachable state above) and then a 301 to `/hello`. `weekendwombat.com` was in exactly that state until 2026-08-25; it now has its own cert. Adding a name here therefore means issuing a cert for it first, otherwise `nginx -t` fails on the missing `fullchain.pem`.
  - **`weekendwolf.app` is the exception: no 443 block, on purpose.** Its DNS points at Cloudflare, not this server, and Cloudflare already answers 410 on both schemes. It stays listed in the port 80 block only so the file documents the full set; requests never arrive here. Verified with `dig` (`104.21.14.48`), not assumed.
  - **`certbot renew` works despite the server-level `return 410`** (the nginx authenticator was verified with `--dry-run` against all four certs). Do not "fix" renewal by moving the 410 into a `location /` to make room for a webroot: that trades a working setup for a weaker guarantee that every path is covered.

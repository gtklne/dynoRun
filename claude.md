...
`src/run/run-state-machine.ts`, `calibration-state-machine.ts`, `session-state-machine.ts` and `calibration-session-state-machine.ts` are pure reducers; the controllers (`run-controller.ts`, `calibration-controller.ts`, `session-controller.ts`, `calibration-session-controller.ts`) own side effects (sensor subscription, repo writes, recorder lifecycle).

```
Run:              idle → ready → running → analyzing → reviewing → saved | aborted
Calibration:      idle → measuring → stable → confirmed
Session:          idle → ready → recording → detecting → reviewing → saving → saved
Calibration sess: idle → ready → recording → detecting → reviewing → saving → saved
```
